...
function SearchForm({ input, setInput, onSubmit, wide }: { input: string; setInput: (v: string) => void; onSubmit: (e: FormEvent) => void; wide: boolean }) {
  return (
    <form onSubmit={onSubmit} className="flex items-center gap-2">
      <div className="relative flex-1">
        ...
        <input
          ...
          className={`${wide ? "w-56" : "w-full"} pl-9 pr-4 py-2.5 rounded-lg bg-[#111c35] border border-[#1e2d52] text-slate-200 placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500/50 transition-all`}
        />
      </div>
      <button
        type="submit"
